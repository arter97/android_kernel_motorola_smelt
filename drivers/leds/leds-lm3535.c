...
unsigned lm3535_read_als_zone(void)
{
	...
	if (lm3535_data.revision <= 1) {
		printk(KERN_ERR
		       "%s: early revison, setting zone to 5 (no ALS)\n",
		       __FUNCTION__);
...
int lm3535_register_als_callback(als_cb func, uint32_t cookie)
{
	...
	if (c == NULL) {
		printk(KERN_ERR
		       "%s: unable to register ALS callback: kzalloc\n",
		       __FUNCTION__);
		return -ENOMEM;
	}
	c->cb = func;
...
static int lm3535_write_reg(unsigned reg, uint8_t value, const char *caller)
{
	...
	printk_write("%s: writing 0x%X to reg 0x%X (%s) at addr 0x%X\n",
		     caller, buf[1], buf[0], reg_name(reg),
		     lm3535_data.client->addr);
				 ...
				 unsigned long action, void *dev)
{
	...
	if (atomic_xchg(&lm3535_data.disp_in_lp, disp_in_lp) != disp_in_lp) {
		...
		if (atomic_read(&lm3535_data.off_at_lp))
			lm3535_brightness_set(&lm3535_led,
					      lm3535_led.brightness);
	}
...
static DEVICE_ATTR(interactive, S_IRUGO | S_IWUSR,
		   lm3535_interactive_show, lm3535_interactive_store);
#endif				/* CONFIG_DOCK_STATUS_NOTIFY */
				      ...
				      const char *buf, size_t size)
{
	...
	if (dim_values[0] != value) {
		dim_values[0] = value;
		if ((lm3535_led.brightness >= 5)
		    && (lm3535_led.brightness < 10))
			lm3535_brightness_set(&lm3535_led,
					      lm3535_led.brightness);
	}
			...
			const struct i2c_device_id *id)
{
	...
	if (lm3535_data.revision > 1) {
		...
		if (client->irq) {
			ret =
			    request_irq(client->irq, lm3535_irq_handler,
					request_flags, "lm3535", &lm3535_data);
			...
			if (ret == 0) {
				...
				printk(KERN_ERR
				       "request_irq %d for lm3535 failed: %d\n",
				       client->irq, ret);
	...
	if (ret) {
		printk(KERN_ERR
		       "%s: unable to create suspend device file for %s: %d\n",
		       __FUNCTION__, lm3535_led.name, ret);
		led_classdev_unregister(&lm3535_led);
		led_classdev_unregister(&lm3535_led_noramp);
	...
	if (lm3535_data.idev == NULL) {
		printk(KERN_ERR
		       "%s: unable to allocate input device file for als\n",
		       __FUNCTION__);
		led_classdev_unregister(&lm3535_led);
		led_classdev_unregister(&lm3535_led_noramp);
		misc_deregister(&als_miscdev);
	...
	if (ret) {
		printk(KERN_ERR
		       "%s: unable to register input device file for als: %d\n",
		       __FUNCTION__, ret);
		led_classdev_unregister(&lm3535_led);
		led_classdev_unregister(&lm3535_led_noramp);
		misc_deregister(&als_miscdev);
...
static void lm3535_work_func(struct work_struct *work)
{
	...
	if (ret) {
		if (reg & ALS_FLAG_MASK) {
			...
			old_zone = atomic_read(&lm3535_data.als_zone);
			printk_als
			    ("%s: ALS zone changed: %d => %d, register = 0x%x\n",
			...
			if (zone > atomic_read(&lm3535_data.bright_zone) ||
				...
				if (!atomic_read(&lm3535_data.in_suspend)) {
					printk_als
					    ("%s: ALS zone increased; changing brightness\n",
					     __FUNCTION__);
					/* Adjust brightness */
					lm3535_brightness_set(&lm3535_led, -1);
				} else {
					printk_als
					    ("%s: ALS zone increased; SUSPEND mode - not changing brightness\n",
					     __FUNCTION__);
				}
			}
			...
			if (old_zone < 2 && zone >= 2) {
				lm3535_write_reg(LM3535_CONFIG_REG,
						 ...
						 __FUNCTION__);
				printk_als
				    ("%s: moved from dim/dark to bright; disable PWM\n",
				     __FUNCTION__);
			} else if (old_zone >= 2 && zone < 2) {
				lm3535_write_reg(LM3535_CONFIG_REG,
						 CONFIG_VALUE | 0x80 |
						 pwm_value, __FUNCTION__);
				printk_als
				    ("%s: moved from bright to dim/dark; enable PWM\n",
				     __FUNCTION__);
			}
			if (!atomic_read(&lm3535_data.in_suspend)) {
			...
			printk_als
			    ("%s: got ALS interrupt but flag is not set: 0x%x\n",
			     __FUNCTION__, reg);
		}
	}
	if (atomic_read(&lm3535_data.do_als_config)) {
		lm3535_write_reg(LM3535_ALS_CTRL_REG, ALS_AVERAGING,
				 __FUNCTION__);
...
static enum hrtimer_restart lm3535_timer_func(struct hrtimer *timer)
{
	...
	hrtimer_start(&lm3535_data.timer, ktime_set(1, 0), HRTIMER_MODE_REL);
...
static int lm3535_enable(struct i2c_client *client, unsigned int on)
{
	...
	if (lm3535_data.revision == 2 || lm3535_data.revision == 3) {
		if (on) {
			ret =
			    lm3535_write_reg(LM3535_TRIM_REG, LM3535_TRIM_VALUE,
					     __FUNCTION__);
		}
	}
	if (!on)
...
static int lm3535_configure(void)
{
	...
	lm3535_write_reg(LM3535_ALS_Z0T_REG, als_zone_max[0], __FUNCTION__);
	lm3535_write_reg(LM3535_ALS_Z1T_REG, als_zone_max[1], __FUNCTION__);
	lm3535_write_reg(LM3535_ALS_Z2T_REG, als_zone_max[2], __FUNCTION__);
	lm3535_write_reg(LM3535_ALS_Z3T_REG, als_zone_max[3], __FUNCTION__);
	lm3535_write_reg(LM3535_ALS_Z4T_REG, als_zone_max[4], __FUNCTION__);
#endif
#endif
	...
	if (atomic_read(&lm3535_data.use_als)) {
		...
		if (ret) {
			...
			atomic_set(&lm3535_data.use_als, 0);
			printk(KERN_ERR
			       "%s: unable to read ALS zone; disabling ALS\n",
			       __FUNCTION__);
		}
	} else {
		atomic_set(&lm3535_data.als_zone, ALS_NO_ZONE);
...
static int lm3535_setup(struct i2c_client *client)
{
	...
	case 0xF0:
		...
		break;
	case 0x02:
		lm3535_data.revision = 2;
		break;
	case 0x00:
		lm3535_data.revision = 3;
		break;
	case 0x01:
		lm3535_data.revision = 4;
		break;
	default:
		lm3535_data.revision = 4;
		break;		// Assume final
...
{
	switch (reg) {
	case (LM3535_ENABLE_REG):
		return "ENABLE";
		break;
	case (LM3535_CONFIG_REG):
		return "CONFIG";
		break;
	case (LM3535_OPTIONS_REG):
		return "OPTIONS";
		break;
	case (LM3535_ALS_REG):
		return "ALS";
		break;
	case (LM3535_ALS_RESISTOR_REG):
		return "ALS_RESISTOR";
		break;
	case (LM3535_ALS_CTRL_REG):
		return "ALS CONTROL";
		break;
	case (LM3535_BRIGHTNESS_CTRL_REG_A):
		return "BRIGHTNESS_CTRL_A";
		break;
	case (LM3535_BRIGHTNESS_CTRL_REG_B):
		return "BRIGHTNESS_CTRL_B";
		break;
	case (LM3535_BRIGHTNESS_CTRL_REG_C):
		return "BRIGHTNESS_CTRL_C";
		break;
	case (LM3535_ALS_ZB0_REG):
		return "ALS_ZB0";
		break;
	case (LM3535_ALS_ZB1_REG):
		return "ALS_ZB1";
		break;
	case (LM3535_ALS_ZB2_REG):
		return "ALS_ZB2";
		break;
	case (LM3535_ALS_ZB3_REG):
		return "ALS_ZB3";
		break;
	case (LM3535_ALS_Z0T_REG):
		return "ALS_Z0T";
		break;
	case (LM3535_ALS_Z1T_REG):
		return "ALS_Z1T";
		break;
	case (LM3535_ALS_Z2T_REG):
		return "ALS_Z2T";
		break;
	case (LM3535_ALS_Z3T_REG):
		return "ALS_Z3T";
		break;
	case (LM3535_ALS_Z4T_REG):
		return "ALS_Z4T";
		break;
	case (LM3535_TRIM_REG):
		return "TRIM";
		break;
	default:
		return "UNKNOWN";
